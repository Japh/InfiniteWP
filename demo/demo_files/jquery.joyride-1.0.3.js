  ...
  $.fn.joyride = function(options) {
    ...
    return this.each(function() {
      ...
      tipLayout = function(tipClass, index, buttonText, self) {
        ...
        (buttonText != '') ? buttonText = '<div class="joyride_footer"><a href="#" class="joyride-next-tip small nice radius yellow button">' + buttonText + '</a></div>': buttonText = '';
        if (settings.inline) {
      ...
      showNextTip = function() {
        ...
        if (count < tipContent.length) {
		  ...
		  if(count==0)
		  	{ $("#joyRidePopup0").css({'position' : 'fixed', 'margin-top' : '-163px', 'margin-left' : '45px', 'width' : '331px'});
				$("#bottom_toolbar").css("z-index","1501");
				$("#ui-tooltip-toggleGroupQtip.top").attr('style','z-index:1502 !important;');
			}
		  else if(count==1)
		  {
		  $("#joyRidePopup1").css({'margin-top' : '-372px', 'margin-left' : '-256px', 'width' : '370px', 'position' : 'fixed'});
		  $("#ui-tooltip-toggleGroupQtip.ui-tooltip").hide();
		  }
		  else if(count==2)
		  {
		  $("#joyRidePopup2").css({'margin-top' : '12px', 'margin-left' : '51px'});
		  $("#bottom_toolbar").css("z-index","1019");
		  $("ul#site_nav").css({'z-index' : '1501', 'box-shadow' : '0 0 14px rgba(0,0,0,0.3)'});
		  }
		  else if(count==3)
		  {
		  $("#joyRidePopup3").css({'margin-top' : '138px', 'margin-left' : '54px'});
		  $("ul#site_nav").css({'z-index' : '1501', 'box-shadow' : '0 0 0px rgba(0,0,0,0)'});
		  $("ul.btn_radio_slelect").css("z-index","1501");
		  $("#WT_updates").css("z-index","1501");
		  
		  
		  }
		  else if(count==4)
		{
		  $("#joyRidePopup4").css({'margin-top' : '161px', 'margin-left' : '-177px'});
		  $("ul.btn_radio_slelect").css("z-index","1499");
		  $(".WT_update_all").css("z-index","1501");
		}
		  else if(count==5)
		  {
		  $("#joyRidePopup5").css({'margin-top' : '384px', 'margin-left' : '768px'});
		  $("#WT_update_all").css("z-index","1499");
		  $(".site_selector1.siteSearch").css("z-index","1501");
		  }
		  else if(count==6)
		  {
		  $("#joyRidePopup6").css({'margin-top' : '23px', 'margin-left' : '-297px', 'width' : '415px'});
		  $(".site_selector1.siteSearch").css("z-index","1499");
		  $("#WT_load_plugins").css("z-index","1501");
		  }
		  else if(count==7)
		  {
		  $("#joyRidePopup7").css({'margin-top' : '173px', 'margin-left' : '593px'});
		  $("#WT_load_plugins").css("z-index","1499");
		  $("#themesInstallBtnWT").css("z-index","1501");
		  $(".btn_rounded_gloss.abs").css("z-index","1501");
		  }
		  else if(count==8)
		  {
		  $("#joyRidePopup8").css({'margin-top' : '-14px', 'margin-left' : '44px'});
		  $("#themesInstallBtnWT").css("z-index","1499");
		  $("#WT_install_theme_details_fav").css("z-index","1501");
		  $(".btn_rounded_gloss.abs").css("z-index","1499");
		  }
		  else if(count==9)
		  {
		  $("#joyRidePopup9").css({'margin-top' : '147px', 'margin-left' : '723px'});
		  $("#WT_install_theme_details_fav").css("z-index","1499");
		  $("#backupPageMainCont").css("z-index","1501");
		  }
		  else if(count==10)
		{
		  $("#joyRidePopup10").css({'margin-top' : '7px', 'margin-left' : '361px'});
		  $("#backupPageMainCont").css("z-index","1499");
		  $("#WT_activity_sample").css("z-index","1501");
		}
		  else if(count==11)
		  {
		  $("#joyRidePopup11").css({'margin-top' : '10px', 'margin-left' : '-160px'});
		  $("#WT_reload_data").css("z-index","1501");
		  $("#WT_activity_sample").css("z-index","1499");
		  $("hr.dotted").css("margin-top","43px");
